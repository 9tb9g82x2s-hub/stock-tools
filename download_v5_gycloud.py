#!/usr/bin/env python3
"""
A股全市场日线数据下载器（gycloud镜像 · trade_date循环 · 三引擎）

走 gycloud HTTP API，用现有token，无需Tushare Pro官方token。
其他跟v5完全一致：按交易日循环、三引擎、重试、速率保护。

用法：
  # SQLite（默认）
  /usr/bin/python3 ~/stock-tools/download_v5_gycloud.py --update

  # MySQL
  /usr/bin/python3 ~/stock-tools/download_v5_gycloud.py \
      --db mysql+pymysql://root:pwd@localhost:3306/stock_db --update

  # MongoDB
  /usr/bin/python3 ~/stock-tools/download_v5_gycloud.py \
      --db mongodb://root:pwd@localhost:27017/stock_db --update

  # 首次全量拉取（2024年全年约220个交易日，预计~2分钟）
  /usr/bin/python3 ~/stock-tools/download_v5_gycloud.py -s 20240101 -e 20241231
"""

import os, sys, time, argparse, json, requests, sqlite3
...
MAX_RETRIES = 3
RETRY_DELAY = 1
RATE_LIMIT_CALLS = 140      # gycloud限制稍严
RATE_LIMIT_WINDOW = 70      # 每70秒140次
COMMIT_EVERY = 20
CALL_GAP = 0.05
...
class DatabaseEngine:
    ...
    def _last_date_sqlite(self):
        self.cur.execute("SELECT MAX(trade_date) FROM daily"); r = self.cur.fetchone()
        return r[0] if r and r[0] else None

    def _commit_sqlite(self): self.conn.commit()
    def _close_sqlite(self): self.conn.commit(); self.cur.close(); self.conn.close()
    ...
    def _init_mysql(self, url):
        ...
        with self.engine.begin() as conn:
            for stmt in MYSQL_DDL.split(";"):
                stmt = stmt.strip()
                if stmt: conn.execute(text(stmt))
        self.raw = self.engine.raw_connection(); self.cur = self.raw.cursor()
        self._engine = "mysql"
    ...
    def _stats_mysql(self):
        self.cur.execute("SELECT COUNT(DISTINCT ts_code) FROM daily"); stocks = self.cur.fetchone()[0]
        self.cur.execute("SELECT MIN(trade_date), MAX(trade_date), COUNT(*) FROM daily"); r = self.cur.fetchone()
        return {"stocks": stocks, "date_min": r[0], "date_max": r[1], "total_rows": r[2]}
    ...
    def _commit_mysql(self): self.raw.commit()
    def _close_mysql(self): self.raw.commit(); self.cur.close(); self.raw.close()
    ...
    def _init_mongo(self, url):
        ...
        self.mongo_coll.create_index(
            [("ts_code", ASCENDING), ("trade_date", ASCENDING)], unique=True, name="idx_ts_code_date")
    ...
    def _insert_mongo(self, records):
        ...
        ops = [ReplaceOne({"ts_code": r[0], "trade_date": r[1]}, dict(zip(FIELD_NAMES, r)), upsert=True) for r in records]
        if ops: self.mongo_coll.bulk_write(ops, ordered=False)

    def _stats_mongo(self):
        stocks = len(self.mongo_coll.distinct("ts_code"))
        pipeline = [{"$group": {"_id": None, "date_min": {"$min": "$trade_date"},
                    "date_max": {"$max": "$trade_date"}, "total": {"$sum": 1}}}]
        r = list(self.mongo_coll.aggregate(pipeline))
        return {"stocks": stocks, "date_min": r[0]["date_min"], "date_max": r[0]["date_max"], "total_rows": r[0]["total"]} if r else {"stocks": 0, "date_min": None, "date_max": None, "total_rows": 0}

    def _last_date_mongo(self):
        doc = self.mongo_coll.find_one({}, sort=[("trade_date", -1)], projection={"trade_date": 1})
        return doc["trade_date"] if doc else None

    def _commit_mongo(self): pass
    def _close_mongo(self): self.mongo_client.close()

    # 统一调度
    def insert_batch(self, records): getattr(self, f"_insert_{self._engine}")(records)
    def commit(self): getattr(self, f"_commit_{self._engine}")()
    def get_last_date(self): return getattr(self, f"_last_date_{self._engine}")()
    def get_stats(self): return getattr(self, f"_stats_{self._engine}")()
    def close(self): getattr(self, f"_close_{self._engine}")()
    @property
    def engine_name(self):
        return {"sqlite": "SQLite", "mysql": "MySQL", "mongodb": "MongoDB"}.get(self._engine, self._engine)


# ============ gycloud HTTP API ============

def call_api(api_name, params, fields):
    """通过gycloud HTTP API调用Tushare接口，带重试"""
...
def download_one_date(trade_date):
    """下载单个交易日全市场日线"""
    items = call_api("daily",
...
def main():
    parser = argparse.ArgumentParser(
        description="gycloud镜像 · trade_date循环 · 三引擎下载器",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s --update                                       # SQLite增量
  %(prog)s -s 20240101 -e 20241231                        # 2024年全年
  %(prog)s --db mysql+pymysql://root:pwd@localhost/stock --update
  %(prog)s --db mongodb://root:pwd@localhost/stock -s 20240101
        """
    )
    parser.add_argument("--db", type=str, default=DEFAULT_DB, help=f"数据库URL (默认: {DEFAULT_DB})")
    ...
    mask_url = args.db
    if "@" in mask_url:
        parts = mask_url.split("@")
        cred = parts[0].split(":") if ":" in parts[0] else [parts[0], "***"]
        mask_url = f"{cred[0]}:***@{'@'.join(parts[1:])}"

    print("=" * 60)
    print("  gycloud镜像 · trade_date循环下载器")
    print("=" * 60)
    print(f"  🗄  引擎: {db.engine_name}  →  {mask_url}")
    print(f"  📅 日期: {start_date} ~ {end_date}")
    print(f"  🔑 Token: {TOKEN[:8]}...{TOKEN[-4:]}")
    print(f"  🌐 数据源: ts.gyzcloud.top")
    print()
    ...
    for i, date in enumerate(dates):
        ...
        records = download_one_date(date); call_count += 1
