...
BASE_DIR = "/Users/ziruzhu/stock-tools/_weekday_4d_run"
OUT_DIR  = "/Users/ziruzhu/stock-tools/_weekday_4d_run"
...
OUT_FILE = f"{OUT_DIR}/s023_weekday_4d_result.json"
...
def load_data():
    ...
    # 只保留必要列，砍掉无关列减小内存(qfq价+特征+trade信息即可)
    keep_cols = ["ts_code","trade_date","open_qfq","close_qfq"] + FEATURE_COLS
    panel = panel[keep_cols].copy()
    panel = panel.sort_values(["ts_code","trade_date"]).reset_index(drop=True)
    g = panel.groupby("ts_code", group_keys=False)
    fwd_ret = g["close_qfq"].transform(lambda s: s.shift(-HORIZON)/s-1)
    med = fwd_ret.groupby(panel["trade_date"]).transform("median")
    panel["label"] = (fwd_ret > med).astype("Int8")
    del fwd_ret, med
    gc.collect()
    ...
    # ★只查喜神池候选股的真实价格(而非全市场)，大幅减小real_px_idx体积
    _con = sqlite3.connect(DB_PATH)
    _rp = ",".join(f"'{d}'" for d in all_dates)
    _codes = ",".join(f"'{c}'" for c in xishen_set)
    _rpx = pd.read_sql(
        f"SELECT ts_code,trade_date,close FROM daily WHERE trade_date IN ({_rp}) AND ts_code IN ({_codes})", _con)
    _con.close()
    ...
    ptd = {all_dates[i+1]: d for i,d in enumerate(all_dates) if i+1<len(all_dates)}
    gc.collect()
...
def run_worker(wd):
    ...
    for k, buy_date in enumerate(buy_days):
        ...
        del model, train_df, score_df  # ★释放大对象
        if k % 50 == 0:
...
def run_master():
    ...
    t0 = time.time()
    log("="*60)
    log(f"S023 真·星期几效应测试: 5个星期几并行(Studio), 持有{HOLD_DAYS}日, 不设止损")
    procs = []
    for wd in range(5):
        ck = f"{OUT_DIR}/wd_signals_{wd}.pkl"
        if os.path.exists(ck):
            log(f"[主控] {WD_NAME[wd]}已有缓存，跳过")
            continue
        p = subprocess.Popen([sys.executable, os.path.abspath(__file__), str(wd)])
        procs.append((wd, p))
        log(f"[主控] 启动{WD_NAME[wd]} PID={p.pid}")
    for wd, p in procs:
        p.wait()
        log(f"[主控] {WD_NAME[wd]} 完成 code={p.returncode}")
