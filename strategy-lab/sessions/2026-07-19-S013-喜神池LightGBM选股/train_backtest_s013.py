...
其他全部沿用S009 v1.5：
  ...
  - C1·-15%收盘价止损（2026-07-19止损优化后默认，原-12%）
  - 买入0.025% + 卖出0.025%+印花税0.05%

断点续跑：每完成12期保存一次checkpoint(ckpt.json)，重启从上次断点继续。
"""
...
XISHEN_PATH = f"{BASE_DIR}/xishen_pool.csv"
CKPT_PATH = f"{BASE_DIR}/ckpt.json"
LOG_PATH = f"{BASE_DIR}/s013_run.log"

BACKTEST_START = "20170101"
TRAIN_MONTHS = 12
TOP_N = 20
STOP_LOSS = -0.15  # 2026-07-19止损优化：从-0.12调至-0.15(年化38.5%/回撤-14.3%/夏普1.58,最贴合低波动稳健定位)
...
def main():
    ...
    result = {
        "strategy_name": "S013 喜神池LightGBM选股(金水1485只)",
    ...
    out = f"{BASE_DIR}/s013_result.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    pd.DataFrame(trades).to_csv(f"{BASE_DIR}/trades_s013.csv", index=False, encoding="utf-8-sig")
    log(f"结果已写出: {out}")
    # 清理断点文件（跑完了不需要了）
    if os.path.exists(CKPT_PATH):
        os.remove(CKPT_PATH)
