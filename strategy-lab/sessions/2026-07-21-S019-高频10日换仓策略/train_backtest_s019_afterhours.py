...
OUT_DIR  = "/Users/ziruzhu/stock-tools/strategy-lab/sessions/2026-07-21-S019-高频10日换仓策略"
...
TOP_N = 20
STOP_LOSS = -0.12
PRICE_LIMIT = 500     # ★S019固化规则：只选真实股价≤500元的股（打分前过滤候选池）
BC, SC = 0.00025, 0.00125

CKPT = f"{OUT_DIR}/ckpt_ah.json"
LOG  = f"{OUT_DIR}/ah_run.log"
OUT_FILE = f"{OUT_DIR}/s019_afterhours_result.json"
...
def main():
    ...
    # ★预加载调仓日真实不复权收盘价（用于股价≤500过滤，close_qfq前复权会失真）
    _con = sqlite3.connect(DB_PATH)
    _rp = ",".join(f"'{d}'" for d in rebalance_dates)
    ...
    log(f"已加载{len(rebalance_dates)}个调仓日的真实收盘价（股价≤{PRICE_LIMIT}元过滤用）")
    ...
    for i, rd in enumerate(rebalance_dates):
        ...
        for code in top20:
            try:
                entry_px = close_lookup.loc[(code, buy_date)]  # 盘后:收盘价买入
