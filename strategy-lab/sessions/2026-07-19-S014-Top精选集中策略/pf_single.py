#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""单cap股价过滤回测(独立进程,跑完即退释放内存)
用法: python pf_single.py <s017|s018> <cap>  cap=99999表示无上限"""
import json, sqlite3, time, os, gc, sys
import numpy as np, pandas as pd
import lightgbm as lgb

SOURCE = sys.argv[1]; CAP = int(sys.argv[2])
if SOURCE=="s018":
    PANEL_PATH="/Users/ziruzhu/stock-tools/strategy-lab/sessions/2026-07-16-S009-LightGBM多因子选股/features_panel.pkl"; USE_XISHEN=False
else:
    PANEL_PATH="/Users/ziruzhu/stock-tools/strategy-lab/sessions/2026-07-19-S013-喜神池LightGBM选股/features_panel.pkl"; USE_XISHEN=True
...
HORIZON=20; BACKTEST_START="20170101"; TRAIN_MONTHS=12; TOP_N=20; STOP_LOSS=-0.12; BC,SC=0.00025,0.00125
...
def month_start(td):
    s=pd.Series(pd.to_datetime(td,format="%Y%m%d")); df=pd.DataFrame({"date":s,"trade_date":td})
    df["ym"]=df["date"].dt.to_period("M"); return df.groupby("ym").first()["trade_date"].tolist()

t0=time.time()
xishen_set=set(pd.read_csv(XISHEN_PATH)["ts_code"]) if USE_XISHEN else None
# 只加载必要列，省内存
...
# Studio有68G内存，不用省内存，保持float64精度
...
all_td=sorted(panel["trade_date"].unique()); reb=[d for d in month_start(all_td) if d>=BACKTEST_START]
...
# 从rebalance_prices.csv读取真实价格（替代DB查询，适配Studio）
...
for i,rd in enumerate(reb):
    ...
    if USE_XISHEN: sc=sc[sc["ts_code"].isin(xishen_set)]
