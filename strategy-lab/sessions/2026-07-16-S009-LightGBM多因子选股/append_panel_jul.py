...
# -*- coding: utf-8 -*-
"""
增量追加 7月特征到 features_panel.pkl（避免全量重建OOM）

思路：现有面板已到6/26有效。只读 2025-01-01 至今的数据（约1年半，
足够120日动量窗口），按build_panel.py同样逻辑算特征，
只保留 trade_date >= 20260627 的新行，追加到现有面板。

铁律：绝不删除/覆盖已有面板行，只做INSERT式追加。先备份原面板。
"""
...
APPEND_FROM = "20260627"  # 只追加这天及以后的新行
LABEL_HORIZON = 10
...
def main():
    ...
    log(f"7月新增交易日: {[d for d in ds if d>='20260701']}")
