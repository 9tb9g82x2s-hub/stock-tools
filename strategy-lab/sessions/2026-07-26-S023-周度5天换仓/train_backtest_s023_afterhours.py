...
# -*- coding: utf-8 -*-
"""
S023 x 盘后固定价格交易 · 方案B回测（afterhours版）
================================================
改造：买入价从"周一开盘"改为"周一收盘"（盘后固定价格交易以收盘价成交）
对比：原版(open买) vs 盘后版(close买)
其余不变：label5天 / 上周五打分 / 周五close卖 / 喜神池 / 止损-12%
"""
...
CKPT = f"{OUT_DIR}/ckpt_afterhours.json"
LOG  = f"{OUT_DIR}/run_afterhours.log"
OUT_FILE = f"{OUT_DIR}/s023_result_afterhours.json"
...
def main():
    t0 = time.time()
    log("="*60)
    log(f"S023盘后版: label{HORIZON}天重训, 上周五打分->周一close买(盘后)->周五close卖")
    ...
    for i, w in enumerate(weeks):
        ...
        for code in top20:
            try:
                entry_px = close_lookup.loc[(code, buy_date)]  # ★周一收盘买（盘后固定价格交易）
    ...
    result = {
        "config":{"horizon":HORIZON,"rebalance":"weekly","buy":"mon_close_afterhours","sell":"fri_close",
