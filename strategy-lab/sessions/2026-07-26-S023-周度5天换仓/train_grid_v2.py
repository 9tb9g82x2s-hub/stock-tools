...
# -*- coding: utf-8 -*-
"""
S023 网格搜索 V2（高效版：训练与出场解耦）
===============================================================================
关键优化 vs V1：
  V1错误：对每种止损线都完整重训454次模型 → 30组=13620次训练=22小时
  V2正确：每个phase只训练一次(得到选股+持有期价格) → 6种止损线在缓存上纯循环模拟
          → 只训练 5 phase × ~454期 = 2270次，比V1快6倍，约3.5小时

回答3个问题：
  1. 分批错开(5phase各1/5资金)能否降回撤？→ combined策略
  2. 哪个相位最优？→ 5个single phase对比
  3. 5天持有最优止损线？→ -6/-8/-10/-12/-15/None 六选一

架构：
  阶段A（慢，训练）：每phase每期，训练LightGBM→选Top20→缓存这20只在[buy_date..sell_date]的
                     开盘价(entry)和逐日收盘价序列。存成 signals[phase] = [{buy,sell,codes,entry_px,daily_close}]
  阶段B（快，模拟）：对每个(phase, stop_loss)，遍历缓存信号，按止损线模拟出场算收益 → 指标

断点续跑：阶段A按phase存checkpoint（signals_phaseN.pkl），跑完的phase不重训
"""
...
DATA_END = "20260717"   # ★剔除20260720~24前复权污染周，数据只到0717
TRAIN_MONTHS = 12
TOP_N = 20
PRICE_LIMIT = 500
...
LOG  = f"{OUT_DIR}/grid_v2_run.log"
OUT_FILE = f"{OUT_DIR}/s023_grid_v2_result.json"
...
def build_signals_for_phase(phase, panel, xishen_set, all_trade_dates,
    ...
    ckpt = f"{OUT_DIR}/signals_phase{phase}.pkl"
...
def main():
    t0 = time.time()
    log("="*60)
    log(f"S023 网格V2(高效版): {N_PHASES}phase × {len(STOP_LOSS_GRID)}止损, 数据截至{DATA_END}(剔污染周)")
